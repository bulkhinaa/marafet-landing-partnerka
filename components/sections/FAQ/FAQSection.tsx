...
const FAQS: { q: string; a: string }[] = [
  ...
  },
  {
    ...
    a: "По доходам от Марафета — нет. Марафет является налоговым агентом по ст. 226 НК РФ. Мы сами удерживаем 13% НДФЛ при каждой выплате и перечисляем в ФНС. Если у вас есть другие доходы (сдача жилья, инвестиции, продажа имущества и т.п.), требующие декларирования — их вы декларируете отдельно, как обычно.",
  },
  {
    q: "А если я уже самозанятый или ИП?",
    a: "Можно совмещать. По умолчанию по партнёрской программе вы работаете как физлицо: Марафет — налоговый агент по НДФЛ и страхователь по взносам. Это не отменяет ваш текущий статус. Если хотите работать по НПД (самозанятости) — обсудим индивидуально с менеджером, для этого есть свои нюансы.",
  },
  {
    q: "Откуда страховые взносы? Я ведь физлицо.",
    a: "По договору ГПХ с физлицом Марафет является вашим страхователем. С каждого начисления платим в фонды полный пакет взносов: пенсионные (ОПС, 22%), медицинские (ОМС, 5,1%), на временную нетрудоспособность (ВНиМ, 2,9%) — всего около 30% от суммы вознаграждения сверх него. У вас идёт страховой стаж и накапливаются пенсионные баллы.",
  },
  {
  ...
  },
  {
    ...
    a: "Привязка реферала к вам не сбрасывается, пока программа действует. Договор бессрочный, расторгается одной из сторон в порядке, описанном в оферте. Если Марафет завершит партнёрскую программу — уведомим заранее, начисления по уже совершённым визитам будут выплачены полностью.",
  },
];
