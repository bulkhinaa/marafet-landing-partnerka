...
const AUDIENCES: Audience[] = [
  {
    ...
    utp: "Привели маникюршу маме, бабушке, подруге — получаете до конца их жизни. Без отрыва от ребёнка и работы.",
    scenario: "Скинула ссылку в семейный чат и районный чат мам, получает на карту",
    anchor: "от 2 500 ₽/мес с 10 рефералов",
    accent: false,
  },
  {
    icon: Smartphone,
    badge: "Блогеры · инфлюенсеры",
    title: "Один пост = доход на годы",
    utp: "В отличие от обычных рекламных интеграций, тут вы получаете не один разовый платёж, а % с каждого визита всех, кто перешёл по ссылке.",
    scenario: "Поставила ссылку в bio, периодически рекомендует в сторис",
    anchor: "от 50 000 ₽/мес с 200 переходов",
    accent: true,
  },
  {
    icon: Sparkles,
    ...
    utp: "+1% к доходу с каждого визита коллег. Без потолка 2,4 млн как у самозанятых. Страховой стаж копится отдельно от вашего основного.",
    scenario: "Привёл коллег из соседнего салона и постоянных клиентов",
    anchor: "от 12 500 ₽/мес с 50 рефералов",
    accent: true,
  },
  {
    icon: Building2,
    ...
    utp: "Параллельно с программой Марафет Пространства. Привели партнёру другой коворкинг, получаете % с каждой записи в нём.",
    scenario: "Рекомендует Марафет коллеге-владельцу из другого города",
    anchor: "от 25 000 ₽/мес с одной точки",
    accent: false,
  },
];
    ...
    <Section anchor="audience" tone="light" className="bg-surface-soft">
      <Container size="xl">
        <SectionHeading
          ...
          subtitle="Один и тот же механизм работает по-разному для разных людей. Найдите свой сценарий."
          align="center"
        />
